...
export default function SeekerDashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <p className="text-slate-500 mb-8">Here are your personalized job recommendations and applications.</p>
      ...
      <section className="mb-10">
        ...
        {recommendations.length === 0 ? (
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-5 text-sm text-yellow-700">
            No recommendations yet. Upload your resume to get AI-based job suggestions 🚀{" "}
            <Link to="/resume" className="underline font-medium">Upload your resume</Link> to get matched with jobs.
          </div>
        ) : (
          <div className="space-y-4">
            {recommendations.map(({ job, match_percentage }) => (
              <div key={job.id} className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-md transition">
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-blue-600">{match_percentage}%</div>
      ...
      {/* My Applications */}
      <section>
        <h2 className="text-lg font-semibold text-slate-700 mb-4">📋 My Applications</h2>
        {applications.length === 0 ? (
          ...
          <div className="space-y-3">
            {applications.map((app) => (
              <div key={app.id} className="bg-white rounded-xl border border-slate-200 p-5 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    ...
                    Match: <span className="text-blue-600 font-medium">{app.match_percentage}%</span>
                  </p>
                </div>
                <span className={`text-xs font-medium px-3 py-1 rounded-full capitalize ${statusColor[app.status]}`}>
                  {app.status}
                </span>
              </div>
            ))}
